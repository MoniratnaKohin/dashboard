import axios from "axios";
 import getAprQuery from "../utils/getAprQuery";
 import { subgraphApiCalls } from "../utils/subgraphApiCalls";
 ...
 export const fetchInsuranceTVL = async () => {
 	const callApis = async (days: number) => {
		...
		console.log("checking response tvl:", response.data.data);
 ...
 export const fetchBetData = async () => {
	console.log("env: ", import.meta.env.VITE_KOHIN_ACCESS_KEY, import.meta.env.VITE_KOHIN_SECRET_KEY);
 ...
 export const fetchTVL = async () => {
 	const query = getAprQuery(import.meta.env.VITE_INSURANCE_POOL_CONTRACT);
 	const response = await subgraphApiCalls(
 		import.meta.env.VITE_SUBGRAPH_API_URL,
 		query
 	);
	console.log("checking response subgraph call:", response);
 	return response.liquidityPoolContracts[0].rawTvl;
 };